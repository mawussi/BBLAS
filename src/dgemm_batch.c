 ...
 * @file dgemm_batch.c
 *
 *  @brief BBLAS dgemm_batch double routine.
 *
...
/**
 * Code generation
 * @generated from ../src/zgemm_batch.c normal z -> d, Mon Jun  6 09:44:14 2016
 **/
#endif
...
/**
    Purpose
    -------
    <b>dgemm_batch</b> is a batch version of dgemm.
    ...
    }else if (batch_opts ==  BBLAS_VARIABLE)
    {
        for (batch_iter = 0; batch_iter < batch_count; batch_iter++)
        {
            ...
            if (M[batch_iter] == 0 || N[batch_iter] == 0 ||
                ((alpha[batch_iter] == (double)0.0 || K[batch_iter] == 0) &&
