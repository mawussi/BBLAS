 ...
 * @file sgemm_batch.c
 *
 *  @brief BBLAS sgemm_batch float routine.
 *
...
/**
 * Code generation
 * @generated from ../src/zgemm_batch.c normal z -> s, Mon Jun  6 09:44:13 2016
 **/
#endif
...
/**
    Purpose
    -------
    <b>sgemm_batch</b> is a batch version of sgemm.
    ...
    }else if (batch_opts ==  BBLAS_VARIABLE)
    {
        for (batch_iter = 0; batch_iter < batch_count; batch_iter++)
        {
            ...
            if (M[batch_iter] == 0 || N[batch_iter] == 0 ||
                ((alpha[batch_iter] == (float)0.0 || K[batch_iter] == 0) &&
