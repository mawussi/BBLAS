/**
 * @file testing_ssymm_batch.c
 *
 * @brief Testing ssymm_batch float routine.
 *
...
/**
 * Code generation
 * @generated from testing_zsymm_batch.c normal z -> s, Mon Jun  6 09:44:14 2016
 **/
#endif
...
void testing_ssymm_batch(bblas_stest_t *test){
	...
	switch (test->target)
	{
	    ...
	    default:
		printf("In testing_ssymm_batch(): Target no defined\n");
		exit(EXIT_FAILURE);
	}
	...
	bblas_ssymm_batch(test->side, test->uplo, test->M, test->N,
	...
	switch(test->target)
	{
		...
		{
		    bblas_ssymm_tolerance(test, test->mkl_result);
		} else
		{
		...
		if(test->new_accuracy)
		{
		    bblas_ssymm_tolerance(test, test->device_result);
		} else
		{
		...
		********************************/
		...
		omp_ssymm_batch(test->side, test->uplo, test->M, test->N,
				test->alpha, (const float**)test->arrayA,
				test->lda, (const float**)test->arrayB,
		...
		if(test->new_accuracy)
		{
		    bblas_ssymm_tolerance(test, test->other_result);
		} else
		{
