/**
 * @file testing_dsyr2k_batch.c
 *
 * @brief Testing dsyr2k_batch double routine.
 *
...
/**
 * Code generation
 * @generated from testing_zsyr2k_batch.c normal z -> d, Mon Jun  6 09:44:14 2016
 **/
#endif
...
void testing_dsyr2k_batch(bblas_dtest_t *test){
	...
	switch (test->target)
	{
	    ...
	    default:
		printf("In testing_dsyr2k_batch(): Target no defined\n");
		exit(EXIT_FAILURE);
	}
	...
	bblas_dsyr2k_batch(test->uplo, test->trans, test->N, test->K,
		    test->alpha, (const double**)test->arrayA,
		    test->lda, (const double**)test->arrayB, test->ldb,
	...
	switch(test->target)
	{
		...
		{
		    bblas_dsyr2k_tolerance(test, test->mkl_result);
		} else
		{
		...
		if(test->new_accuracy)
		{
		    bblas_dsyr2k_tolerance(test, test->device_result);
		} else
		{
		...
		********************************/
		...
		omp_dsyr2k_batch(test->uplo, test->trans, test->N, test->K,
			    test->alpha, (const double**)test->arrayA,
			    test->lda, (const double**)test->arrayB, test->ldb,
		...
		if(test->new_accuracy)
		{
		    bblas_dsyr2k_tolerance(test, test->other_result);
		} else
		{
