...
void bblas_zprint_result(bblas_ztest_t *test)
{
	...
	case BBLAS_MKL:
	    ...
	    bblas_zpassed_failed(test, test->mkl_max_error, test_passed_failed, error_in_info);
    ...
    case BBLAS_MAGMA:
	    ...
	    bblas_zpassed_failed(test, test->device_max_error, test_passed_failed, error_in_info);
	...
	case BBLAS_OTHER:
	    ...
	    bblas_zpassed_failed(test, test->other_max_error, test_passed_failed, error_in_info);
	...
	case BBLAS_CUMKL:
	    ...
	    break;
	default:
	    printf("In bblas_zprint_result(): Target not defined\n");
	    exit(EXIT_FAILURE);
    }
}
...
void bblas_zprint_parameters(bblas_ztest_t *test)
{
    ...
    char funcname[] = "bblas_zprint_parameters";
    char separator[] = "--";
    char  col[]="|";
    int first_index =0;
	char* transA;
