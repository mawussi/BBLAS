/**
 * @file testing_ssyrk_batch.c
 *
 * @brief Testing ssyrk_batch float routine.
 *
...
/**
 * Code generation
 * @generated from testing_zsyrk_batch.c normal z -> s, Mon Jun  6 09:44:13 2016
 **/
#endif
...
void testing_ssyrk_batch(bblas_stest_t *test){
	...
	switch (test->target)
	{
	    ...
	    default:
		printf("In testing_ssyrk_batch(): Target no defined\n");
		exit(EXIT_FAILURE);
	}
	...
	bblas_ssyrk_batch(test->uplo, test->trans, test->N, test->K,
	...
	switch(test->target)
	{
		...
		{
		    bblas_ssyrk_tolerance(test, test->mkl_result);
		} else
		{
		...
		if(test->new_accuracy)
		{
		    bblas_ssyrk_tolerance(test, test->device_result);
		} else
		{
		...
		********************************/
		...
		omp_ssyrk_batch(test->uplo, test->trans, test->N, test->K,
			    test->alpha, (const float**)test->arrayA,
		...
		if(test->new_accuracy)
		{
		    bblas_ssyrk_tolerance(test, test->other_result);
		} else
		{
