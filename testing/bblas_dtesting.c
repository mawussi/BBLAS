/**
 * @file bblas_dtesting.c
 *
 * @brief BBLAS testing  for double routines.
 *
...
/**
 * Code generation
 * @generated from bblas_ztesting.c normal z -> d, Mon Jun  6 09:44:14 2016
 **/
#endif
...
void bblas_dsettest(bblas_dtest_t *test)
{
    ...
    {
	bblas_dsetbeta_herk(test);
    }

    /*
     * Allocate memory and set values for arrayA
     */
    bblas_dsetarrayA(test);
	...
	(routine == BBLAS_HER2K))
    {
	bblas_dsetarrayC(test);
    }

    /* Memory allocation for result and error variables */
    bblas_dmalloc_result_error(test);
}
...
void bblas_dsetalpha(bblas_dtest_t *test)
{
    ...
    test->alpha = (double*) malloc(batch_count*sizeof(double));
    ...
    {
	test->alpha[batch_iter] = ((double)rand()/(double)RAND_MAX);
    }
}
...
void bblas_dsetbeta(bblas_dtest_t *test)
{
    ...
    test->beta = (double*) malloc(batch_count*sizeof(double));
    ...
    for( batch_iter =0; batch_iter < batch_count; batch_iter++)
    {
	test->beta[batch_iter] = ((double)rand()/(double)RAND_MAX);
    }
}
...
void bblas_dsetarrayA(bblas_dtest_t *test)
{
    int batch_iter, nb_row, nb_col;
     ...
     test->arrayA = (double**) malloc(batch_count*sizeof(double*));
	 ...
	 for( batch_iter =0; batch_iter < batch_count; batch_iter++)
	 {
	     ...
	     /*Matrix filling */
	     test->arrayA[batch_iter] = (double *) malloc(nb_row*nb_col* sizeof(double ));
	     bblas_malloc_check(test->arrayA[batch_iter], ptr_name);

#if defined(BBLAS_WITH_MKL)
	     LAPACKE_dlarnv_work(IONE, ISEED, nb_row*nb_col, (double*) test->arrayA[batch_iter]);
#else
		 LAPACKE_dlarnv_work(IONE, ISEED, nb_row*nb_col, test->arrayA[batch_iter]);
#endif
...
void bblas_dsetarrayB(bblas_dtest_t *test)
{
    ...
    test->arrayB = (double**) malloc(batch_count*sizeof(double*));
	...
	for( batch_iter =0; batch_iter < batch_count ; batch_iter++)
	{
	    ...
	    test->arrayB[batch_iter] = (double *) malloc(nb_row*nb_col* sizeof(double ));
	    bblas_malloc_check(test->arrayB[batch_iter], ptr_name);

#if defined(BBLAS_WITH_MKL)
	    LAPACKE_dlarnv_work(IONE, ISEED, nb_row*nb_col, (double*) test->arrayB[batch_iter]);
#else
		LAPACKE_dlarnv_work(IONE, ISEED, nb_row*nb_col, test->arrayB[batch_iter]);
#endif
	    ...
	    {
#if defined(BBLAS_WITH_MKL)
		test->Binitnorm[batch_iter] = (double) LAPACKE_dlange_work(LAPACK_COL_MAJOR,
								  'I', nb_row, nb_col,
								  (double*) test->arrayB[batch_iter],
								  test->ldb[batch_iter], work);
#else
		test->Binitnorm[batch_iter] = LAPACKE_dlange_work(LAPACK_COL_MAJOR,
								  'I', nb_row, nb_col,
...
void bblas_dsetarrayC(bblas_dtest_t *test)
{
    ...
    test->arrayC = (double**) malloc(batch_count*sizeof(double*));
	...
	for( batch_iter =0; batch_iter < batch_count ; batch_iter++)
	{
	    ...
	    test->arrayC[batch_iter] = (double *) malloc(nb_row*nb_col* sizeof(double ));
	    bblas_malloc_check(test->arrayC[batch_iter], ptr_name);
#if defined(BBLAS_WITH_MKL)
	    LAPACKE_dlarnv_work(IONE, ISEED, nb_row*nb_col, (double*) test->arrayC[batch_iter]);
#else
		LAPACKE_dlarnv_work(IONE, ISEED, nb_row*nb_col, test->arrayC[batch_iter]);
#endif
...
#if defined(BBLAS_WITH_MKL)
	    test->Cinitnorm[batch_iter] = (double) LAPACKE_dlange_work(LAPACK_COL_MAJOR,
								  'I', nb_row, nb_col, (double*) test->arrayC[batch_iter],
								  test->ldc[batch_iter], work);
#else
		test->Cinitnorm[batch_iter] = LAPACKE_dlange_work(LAPACK_COL_MAJOR,
...
void bblas_dmalloc_result_error(bblas_dtest_t *test)
{
    ...
    {
	case BBLAS_MKL:
	    test->mkl_result  = (double**) malloc(batch_count*sizeof(double*));
	...
	case BBLAS_CUBLAS:
	case BBLAS_MAGMA:
	    test->device_result = (double**) malloc(batch_count*sizeof(double*));
	...
	case BBLAS_OTHER:
	    test->other_result  = (double**) malloc(batch_count*sizeof(double*));
...
void bblas_dcopy_Cinit(bblas_dtest_t *test, double **C_copy)
{
    ...
    if (!((routine == BBLAS_GEMM) || (routine == BBLAS_SYMM) ||
	  ...
	  (routine == BBLAS_HER2K)))
    {
	printf("BBLAS FATAL ERROR: bblas_dtesting.c():\n");
	printf("\t bblas_dcopy_Cinit not defined for %s \n", bblas_getroutine(test->routine));
	exit(EXIT_FAILURE);
    }
	...
	for( batch_iter =0; batch_iter < batch_count ; batch_iter++)
	{
	    ...
	    N   = test->N[batch_iter];
	    C_copy[batch_iter] = (double *) malloc(ldc*N* sizeof(double ));

	    /*Copy the matrix {C}_i */
	    cblas_dcopy (ldc*N, test->arrayC[batch_iter], 1, C_copy[batch_iter], 1);
	}
...
void bblas_dcopy_Binit(bblas_dtest_t *test, double **B_copy)
{
    ...
    {
	printf("BBLAS FATAL ERROR: bblas_dtesting.c():\n");
	printf("\t bblas_dcopy_Binit not defined for %s \n", bblas_getroutine(test->routine));
	exit(EXIT_FAILURE);
    }
	...
	for( batch_iter =0; batch_iter < batch_count ; batch_iter++)
	{
	    ...
	    B_copy[batch_iter] = (double *) malloc(ldb*N* sizeof(double ));

	    /*Copy the matrix {B}_i */
	    cblas_dcopy (ldb*N, test->arrayB[batch_iter], 1, B_copy[batch_iter], 1);
	}
...
void bblas_dcheck_Cfinal(bblas_dtest_t *test, double **C_final)
{
    ...
    {
	printf("BBLAS FATAL ERROR: bblas_dtesting.c():\n");
	printf("\t bblas_dcheck_Cfinal not defined for %s \n", bblas_getroutine(test->routine));
	exit(EXIT_FAILURE);
    }
...
void bblas_dcheck_Bfinal(bblas_dtest_t *test, double **B_final)
{
    ...
    {
	printf("BBLAS FATAL ERROR: bblas_dtesting.c():\n");
	printf("\t bblas_dcheck_Bfinal not defined for %s \n", bblas_getroutine(test->routine));
	exit(EXIT_FAILURE);
    }
	...
	for( batch_iter =0; batch_iter < batch_count ; batch_iter++)
	{
	    ...
	    Bnorm = cblas_dnrm2(ldb*N, test->arrayB[batch_iter], 1);

	    /*Compute the error */
	    cblas_daxpy (ldb*N, (alpha), B_final[batch_iter], 1, B_diff[batch_iter], 1);

	    /*Compute  the norm assoicated  with the error */
	    Error_norm = cblas_dnrm2(ldb*N, B_diff[batch_iter], 1);
		...
		case BBLAS_OTHER:
		    ...
		    break;
		default:
		    printf("In bblas_dcheck_Bfinal(): Target no defined\n");
		    exit(EXIT_FAILURE);
	    }
	}
...
void bblas_dfreetest( bblas_dtest_t *test )
{
	...
	default:
	    printf("In bblas_dfreetest(): Target no defined\n");
	    exit(EXIT_FAILURE);
    }
    /*Free INFO */
    free( test->info );
