/**
 * @file omp_dgemm_batch.c
 *
 * @brief BBLAS gemm_batch double routine.
 *
...
/**
 * Code generation
 * @generated from ./bblas_omp/omp_zgemm_batch.c normal z -> d, Mon Jun  6 09:44:14 2016
 **/
#endif

#include<cblas.h>
#include "bblas_domp.h"
#include "bblas.h"
#include <omp.h>
#define REAL
...
/**
    Purpose
    -------
    <b>omp_dgemm_batch</b> is an OpenMP version of dgemm_batch.
    ...
    info    Array of <tt>int</tt>.
            Each element info[i] is the error return code of the ith dgemm in the batch,
...
/*Local variables  */
    ...
    if (batch_opts == BBLAS_FIXED)
    {
        ...
        if (M[first_index] == 0 || N[first_index] == 0     ||
	        ((alpha[first_index] == (double)0.0 || K[first_index] == 0) &&
...
#pragma omp parallel for private(LDA, LDB)
        for (int batch_iter_omp = 0; batch_iter_omp < batch_count; batch_iter_omp++)
        {
            ...
            if (M[batch_iter_omp] == 0 || N[batch_iter_omp] == 0 ||
                ((alpha[batch_iter_omp] == (double)0.0 || K[batch_iter_omp] == 0) &&
                beta[batch_iter_omp] == (double)1.0))
            {
                info[batch_iter_omp] =  BBLAS_SUCCESS;
                continue;
            }
