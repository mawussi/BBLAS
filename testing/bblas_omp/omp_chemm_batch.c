/**
 * @file omp_chemm_batch.c
 *
 * @brief BBLAS omp_chemm_batch float _Complex routine.
 *
...
/**
 * Code generation
 * @generated from ./bblas_omp/omp_zhemm_batch.c normal z -> c, Mon Jun  6 09:44:14 2016
 **/
#endif
...
/**
    Purpose
    -------
    <b>chemm_batch</b> is an OpenMP version of chemm_batch.
    ...
    where alpha[i] and beta[i] are scalars, arrayA[i] is a hermitian matrix,
    and arrayB[i] and C are M[i] by N[i] matrices.
    ...
    side    Array of <tt>enum BBLAS_SIDE</tt>.
            On entry, side[i] specifies whether the hermitian
    ...
    M       Array of <tt>int</tt>.
            ...
            arrayC[i]. M[i] must be greater than zero.
    ...
    arrayA   Array of pointers.
			 ...
			 When using side[i] = BblasLeft the M[i] by M[i] part of arrayA[i] must
			 contain the Hermitian matrix:
			 when uplo[i] = BblasUpper, the upper triangular part of arrayA[i]
			 must contain the upper triangular part of the Hermitian matrix whilst
			 the strictly lower triangular part is not used;
			 similarly when uplo[i] = BblasLower, the lower triangular part of arrayA[i]
			 must contain the lower triangular part of the Hermitian matrix whilst
			 the strictly upper triangular part is not used.
    ...
    arrayC  Array of pointers.
			Each element arrayC[i] is a pointer to a COMPLEX matrix of
    ...
    info   Array of <tt>int</tt>.
	        Each element info[i] is the error return code of the ith chemm in the batch,
...
/*Local variables  */
	...
	if (M[first_index] == 0 || N[first_index] == 0 ||
	    ((alpha[first_index] == (BBLAS_Complex32_t)0.0) &&
	     (beta[first_index] == (BBLAS_Complex32_t)1.0)))
	{
	    for (batch_iter = 0; batch_iter < batch_count; batch_iter++)
	    {
...
#pragma omp parallel for private(batch_iter, LDA)
		...
		(uplo[batch_iter] != BblasLower))
            ...
            if (M[batch_iter] == 0 || N[batch_iter] == 0 ||
                ((alpha[batch_iter] == (BBLAS_Complex32_t)0.0) &&
		 beta[batch_iter] == (BBLAS_Complex32_t)1.0))
            {
                info[batch_iter] =  BBLAS_SUCCESS;
                continue;
            }
