/**
 *
 * @file testing_chemm_batch.c
 *
 * @brief Testing chemm_batch float _Complex routine.
 *
...
/**
 * Code generation
 * @generated from testing_zhemm_batch.c normal z -> c, Mon Jun  6 09:44:13 2016
 **/
#endif
...
void testing_chemm_batch(bblas_ctest_t *test){
	...
	switch (test->target)
	{
	    ...
	    default:
		printf("In testing_chemm_batch(): Target no defined\n");
		exit(EXIT_FAILURE);
	}
	...
	bblas_chemm_batch(test->side, test->uplo, test->M, test->N,
	...
	switch(test->target)
	{
		...
		{
		    bblas_chemm_tolerance(test, test->mkl_result);
		} else
		{
		...
		if(test->new_accuracy)
		{
		    bblas_chemm_tolerance(test, test->device_result);
		} else
		{
		...
		********************************/
		...
		omp_chemm_batch(test->side, test->uplo, test->M, test->N,
				test->alpha, (const BBLAS_Complex32_t**)test->arrayA,
				test->lda, (const BBLAS_Complex32_t**)test->arrayB,
		...
		if(test->new_accuracy)
		{
		    bblas_chemm_tolerance(test, test->other_result);
		} else
		{
