/**
 * @file testing_zsyrk_batch.c
 *
 * @brief Testing zsyrk_batch double _Complex routine.
 *
...
void testing_zsyrk_batch(bblas_ztest_t *test){
	...
	switch (test->target)
	{
	    ...
	    default:
		printf("In testing_zsyrk_batch(): Target no defined\n");
		exit(EXIT_FAILURE);
	}
	...
	bblas_zsyrk_batch(test->uplo, test->trans, test->N, test->K,
	...
	switch(test->target)
	{
		...
		{
		    bblas_zsyrk_tolerance(test, test->mkl_result);
		} else
		{
		...
		if(test->new_accuracy)
		{
		    bblas_zsyrk_tolerance(test, test->device_result);
		} else
		{
		...
		********************************/
		...
		omp_zsyrk_batch(test->uplo, test->trans, test->N, test->K,
			    test->alpha, (const BBLAS_Complex64_t**)test->arrayA,
		...
		if(test->new_accuracy)
		{
		    bblas_zsyrk_tolerance(test, test->other_result);
		} else
		{
