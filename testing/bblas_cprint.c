 ...
 * @file bblas_cprint.c
 *
 * @brief BBLAS printing routines for float _Complex routines.
 *
...
/**
 * Code generation
 * @generated from bblas_zprint.c normal z -> c, Mon Jun  6 09:44:14 2016
 **/
#endif
...
void bblas_cprint_result(bblas_ctest_t *test)
{
	...
	bblas_print_time();
	bblas_cprint_sys_info(test);
	bblas_print_tolerance(test->routine, test->new_accuracy);
	#ifdef COMPLEX
	printf("Note: Gflop/s is reported in real flops: one complex addition is equal to two real additions etc.\n");
	#endif
	...
	case BBLAS_MKL:
	    ...
	    bblas_cpassed_failed(test, test->mkl_max_error, test_passed_failed, error_in_info);
    ...
    case BBLAS_MAGMA:
	    ...
	    bblas_cpassed_failed(test, test->device_max_error, test_passed_failed, error_in_info);
	...
	case BBLAS_OTHER:
	    ...
	    bblas_cpassed_failed(test, test->other_max_error, test_passed_failed, error_in_info);
	...
	case BBLAS_CUMKL:
	    ...
	    break;
	default:
	    printf("In bblas_cprint_result(): Target not defined\n");
	    exit(EXIT_FAILURE);
    }
}
...
void bblas_cprint_parameters(bblas_ctest_t *test)
{
    ...
    char funcname[] = "bblas_cprint_parameters";
    char separator[] = "--";
    char  col[]="|";
    int first_index =0;
	char* transA;
