/**
 * @file testing_ssyr2k_batch.c
 *
 * @brief Testing ssyr2k_batch float routine.
 *
...
/**
 * Code generation
 * @generated from testing_zsyr2k_batch.c normal z -> s, Mon Jun  6 09:44:14 2016
 **/
#endif
...
void testing_ssyr2k_batch(bblas_stest_t *test){
	...
	switch (test->target)
	{
	    ...
	    case BBLAS_CUBLAS:
		bblas_scopy_Cinit(test, test->device_result);
		break;
	    ...
	    default:
		printf("In testing_ssyr2k_batch(): Target no defined\n");
		exit(EXIT_FAILURE);
	}
	...
	bblas_ssyr2k_batch(test->uplo, test->trans, test->N, test->K,
		    test->alpha, (const float**)test->arrayA,
		    test->lda, (const float**)test->arrayB, test->ldb,
		    test->beta, test->arrayC, test->ldc,
	...
	switch(test->target)
	{
		...
		bblas_ssyr2k_batch(test->uplo, test->trans, test->N, test->K,
		    test->alpha, (const float**)test->arrayA,
		    test->lda, (const float**)test->arrayB, test->ldb,
		    test->beta, test->mkl_result, test->ldc,
		    test->batch_count, test->batch_opts, test->info);
		...
		{
		    bblas_ssyr2k_tolerance(test, test->mkl_result);
		} else
		{
		...
		bblas_ssyr2k_batch(test->uplo, test->trans, test->N, test->K,
			    test->alpha, (const float**)test->arrayA,
			    test->lda, (const float**)test->arrayB, test->ldb,
			    test->beta, test->device_result, test->ldc,
			    test->batch_count, test->batch_opts, test->info);
		...
		if(test->new_accuracy)
		{
		    bblas_ssyr2k_tolerance(test, test->device_result);
		} else
		{
		...
		********************************/
		...
		omp_ssyr2k_batch(test->uplo, test->trans, test->N, test->K,
			    test->alpha, (const float**)test->arrayA,
			    test->lda, (const float**)test->arrayB, test->ldb,
			    test->beta, test->other_result, test->ldc,
			    test->batch_count, test->batch_opts, test->info);
		...
		if(test->new_accuracy)
		{
		    bblas_ssyr2k_tolerance(test, test->other_result);
		} else
		{
