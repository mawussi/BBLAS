/**
 * @file testing_csyrk_batch.c
 *
 * @brief Testing csyrk_batch float _Complex routine.
 *
...
/**
 * Code generation
 * @generated from testing_zsyrk_batch.c normal z -> c, Mon Jun  6 09:44:14 2016
 **/
#endif
...
void testing_csyrk_batch(bblas_ctest_t *test){
	...
	switch (test->target)
	{
	    ...
	    default:
		printf("In testing_csyrk_batch(): Target no defined\n");
		exit(EXIT_FAILURE);
	}
	...
	bblas_csyrk_batch(test->uplo, test->trans, test->N, test->K,
	...
	switch(test->target)
	{
		...
		{
		    bblas_csyrk_tolerance(test, test->mkl_result);
		} else
		{
		...
		if(test->new_accuracy)
		{
		    bblas_csyrk_tolerance(test, test->device_result);
		} else
		{
		...
		********************************/
		...
		omp_csyrk_batch(test->uplo, test->trans, test->N, test->K,
			    test->alpha, (const BBLAS_Complex32_t**)test->arrayA,
		...
		if(test->new_accuracy)
		{
		    bblas_csyrk_tolerance(test, test->other_result);
		} else
		{
