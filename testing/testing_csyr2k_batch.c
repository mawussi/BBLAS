/**
 * @file testing_csyr2k_batch.c
 *
 * @brief Testing csyr2k_batch float _Complex routine.
 *
...
/**
 * Code generation
 * @generated from testing_zsyr2k_batch.c normal z -> c, Mon Jun  6 09:44:14 2016
 **/
#endif
...
void testing_csyr2k_batch(bblas_ctest_t *test){
	...
	switch (test->target)
	{
	    ...
	    default:
		printf("In testing_csyr2k_batch(): Target no defined\n");
		exit(EXIT_FAILURE);
	}
	...
	bblas_csyr2k_batch(test->uplo, test->trans, test->N, test->K,
	...
	switch(test->target)
	{
		...
		{
		    bblas_csyr2k_tolerance(test, test->mkl_result);
		} else
		{
		...
		if(test->new_accuracy)
		{
		    bblas_csyr2k_tolerance(test, test->device_result);
		} else
		{
		...
		********************************/
		...
		omp_csyr2k_batch(test->uplo, test->trans, test->N, test->K,
			    test->alpha, (const BBLAS_Complex32_t**)test->arrayA,
			    test->lda, (const BBLAS_Complex32_t**)test->arrayB, test->ldb,
		...
		if(test->new_accuracy)
		{
		    bblas_csyr2k_tolerance(test, test->other_result);
		} else
		{
