/**
 * @file testing_ctrmm_batch.c
 *
 * @brief Testing ctrmm_batch float _Complex routine.
 *
...
/**
 * Code generation
 * @generated from testing_ztrmm_batch.c normal z -> c, Mon Jun  6 09:44:14 2016
 **/
#endif
...
void testing_ctrmm_batch(bblas_ctest_t *test){
	...
	switch (test->target)
	{
	    ...
	    case BBLAS_CUBLAS:
		bblas_ccopy_Binit(test, test->device_result);
		break;
	    ...
	    default:
		printf("In testing_ctrmm_batch(): Target no defined\n");
		exit(EXIT_FAILURE);
	}
	...
	bblas_ctrmm_batch(test->side, test->uplo, test->transA, test->diag,
	...
	switch(test->target)
	{
		...
		{
		    bblas_ctrmm_tolerance(test, test->mkl_result);
		} else
		{
		...
		if(test->new_accuracy)
		{
		    bblas_ctrmm_tolerance(test, test->device_result);
		} else
		{
		...
		********************************/
		...
		omp_ctrmm_batch(test->side, test->uplo, test->transA, test->diag,
			    test->M, test->N, test->alpha, (const BBLAS_Complex32_t**)test->arrayA,
		...
		if(test->new_accuracy)
		{
		    bblas_ctrmm_tolerance(test, test->other_result);
		} else
		{
