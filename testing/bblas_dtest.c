 ...
 * @file bblas_dtest.c
 *
 * @brief Runs tests for double routines.
 *
...
/**
 * Code generation
 * @generated from bblas_ztest.c normal z -> d, Mon Jun  6 09:44:14 2016
 **/
#endif
...
int main(int argc, char **argv){
    ...
    /* Local variables */
    bblas_dtest_t *test       = ( bblas_dtest_t *) malloc (sizeof(bblas_dtest_t));
    ...
    bblas_dinit_config (test);

    /*Read parameters  */
    parse_dconfigfile(file, test);
    /* Close the file */
    fclose( file );
	...
	case BBLAS_GEMM:
	    ...
	    testing_dgemm_batch(test);
	    break;

	case BBLAS_HEMM:
	    testing_dsymm_batch(test);
	    break;

	case BBLAS_HER2K:
	    testing_dsyr2k_batch(test);
	    break;

	case BBLAS_HERK:
	    testing_dsyrk_batch(test);
	    break;
	...
	case BBLAS_TRMM:
	    testing_dtrmm_batch(test);
	    break;

	case BBLAS_TRSM:
	    testing_dtrsm_batch(test);
	    break;
