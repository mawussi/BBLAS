/**
 * @file testing_ztrmm_batch.c
 *
 * @brief Testing ztrmm_batch double _Complex routine.
 *
...
void testing_ztrmm_batch(bblas_ztest_t *test){
	...
	switch (test->target)
	{
	    ...
	    case BBLAS_CUBLAS:
		bblas_zcopy_Binit(test, test->device_result);
		break;
	    ...
	    default:
		printf("In testing_ztrmm_batch(): Target no defined\n");
		exit(EXIT_FAILURE);
	}
	...
	bblas_ztrmm_batch(test->side, test->uplo, test->transA, test->diag,
	...
	switch(test->target)
	{
		...
		{
		    bblas_ztrmm_tolerance(test, test->mkl_result);
		} else
		{
		...
		if(test->new_accuracy)
		{
		    bblas_ztrmm_tolerance(test, test->device_result);
		} else
		{
		...
		********************************/
		...
		omp_ztrmm_batch(test->side, test->uplo, test->transA, test->diag,
			    test->M, test->N, test->alpha, (const BBLAS_Complex64_t**)test->arrayA,
		...
		if(test->new_accuracy)
		{
		    bblas_ztrmm_tolerance(test, test->other_result);
		} else
		{
