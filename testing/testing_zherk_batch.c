 ...
 *
 * @brief Testing zherk_batch double _Complex routine.
 *
...
void testing_zherk_batch(bblas_ztest_t *test){
  ...
  cublasCreate(&test->cublas_handle);
	...
	switch (test->target)
	{
	    ...
	    default:
		printf("In testing_zherk_batch(): Target no defined\n");
		exit(EXIT_FAILURE);
	}
	...
	bblas_zherk_batch(test->uplo, test->trans, test->N, test->K,
		    test->alpha_herk, (const BBLAS_Complex64_t**)test->arrayA,
	...
	switch(test->target)
	{
		...
		{
		    bblas_zherk_tolerance(test, test->mkl_result);
		} else
		{
		...
		if(test->new_accuracy)
		{
		    bblas_zherk_tolerance(test, test->device_result);
		} else
		{
		...
		magma_zherk_batch(test->uplo, test->trans, test->N, test->K,
			    test->alpha_herk, (const BBLAS_Complex64_t**)test->arrayA_device,
		...
		********************************/
		...
		omp_zherk_batch(test->uplo, test->trans, test->N, test->K,
			    test->alpha_herk, (const BBLAS_Complex64_t**)test->arrayA,
		...
		if(test->new_accuracy)
		{
		    bblas_zherk_tolerance(test, test->other_result);
		} else
		{
