/**
 * @file testing_dsymm_batch.c
 *
 * @brief Testing dsymm_batch double routine.
 *
...
/**
 * Code generation
 * @generated from testing_zsymm_batch.c normal z -> d, Mon Jun  6 09:44:14 2016
 **/
#endif
...
void testing_dsymm_batch(bblas_dtest_t *test){
	...
	switch (test->target)
	{
	    ...
	    default:
		printf("In testing_dsymm_batch(): Target no defined\n");
		exit(EXIT_FAILURE);
	}
	...
	bblas_dsymm_batch(test->side, test->uplo, test->M, test->N,
	...
	switch(test->target)
	{
		...
		{
		    bblas_dsymm_tolerance(test, test->mkl_result);
		} else
		{
		...
		if(test->new_accuracy)
		{
		    bblas_dsymm_tolerance(test, test->device_result);
		} else
		{
		...
		********************************/
		...
		omp_dsymm_batch(test->side, test->uplo, test->M, test->N,
				test->alpha, (const double**)test->arrayA,
				test->lda, (const double**)test->arrayB,
		...
		if(test->new_accuracy)
		{
		    bblas_dsymm_tolerance(test, test->other_result);
		} else
		{
