/**
 *
 * @file testing_zhemm_batch.c
 *
 * @brief Testing zhemm_batch double _Complex routine.
 *
...
void testing_zhemm_batch(bblas_ztest_t *test){
	...
	switch (test->target)
	{
	    ...
	    default:
		printf("In testing_zhemm_batch(): Target no defined\n");
		exit(EXIT_FAILURE);
	}
	...
	bblas_zhemm_batch(test->side, test->uplo, test->M, test->N,
	...
	switch(test->target)
	{
		...
		{
		    bblas_zhemm_tolerance(test, test->mkl_result);
		} else
		{
		...
		if(test->new_accuracy)
		{
		    bblas_zhemm_tolerance(test, test->device_result);
		} else
		{
		...
		********************************/
		...
		omp_zhemm_batch(test->side, test->uplo, test->M, test->N,
				test->alpha, (const BBLAS_Complex64_t**)test->arrayA,
				test->lda, (const BBLAS_Complex64_t**)test->arrayB,
		...
		if(test->new_accuracy)
		{
		    bblas_zhemm_tolerance(test, test->other_result);
		} else
		{
