/**
 * @file testing_cgemm_batch.c
 *
 * @brief Testing cgemm_batch float _Complex routine.
 *
...
/**
 * Code generation
 * @generated from testing_zgemm_batch.c normal z -> c, Mon Jun  6 09:44:14 2016
 **/
#endif
...
void testing_cgemm_batch(bblas_ctest_t *test){
	...
	default:
	  printf("In testing_cgemm_batch(): Target no defined\n");
	  exit(EXIT_FAILURE);
	}
	...
	bblas_cgemm_batch(test->transA, test->transB, test->M, test->N, test->K,
	...
	switch(test->target)
	{
		...
		/* =====================================================================
		   ...
		   =================================================================== */
			...
			mkl_cgemm_batch(test->transA, test->transB,
					 test->M,  test->N, test->K,
					 test->alpha,
					 (const BBLAS_Complex32_t**) test->arrayA, test->lda,
					 (const BBLAS_Complex32_t**) test->arrayB, test->ldb,
					 test->beta,
			...
			{
			    bblas_cgemm_tolerance(test, test->mkl_result);
			} else
			{
		...
		/* =====================================================================
	      ...
	      cublas_cgemm_batch(test->transA, test->transB,
				 test->M, test->N, test->K,
				 test->alpha,
				 (const BBLAS_Complex32_t**) test->arrayA_device, test->lda,
				 (const BBLAS_Complex32_t**) test->arrayB_device, test->ldb,
				 test->beta,
		...
		/*Compute relative errors */
	      if(test->new_accuracy)
		{
		    bblas_cgemm_tolerance(test, test->device_result);
		}
	      else
		{
		...
		/* =====================================================================
	      ...
	      magma_cgemm_batch(test->transA, test->transB,
				test->M, test->N, test->K,
				test->alpha,
				(const BBLAS_Complex32_t**) test->arrayA_device, test->lda,
				(const BBLAS_Complex32_t**) test->arrayB_device, test->ldb,
	...
	case BBLAS_CUMKL:
		...
		omp_cgemm_batch(test->transA, test->transB, test->M, test->N, test->K,
				test->alpha, (const BBLAS_Complex32_t**)test->arrayA,
				test->lda, (const BBLAS_Complex32_t**)test->arrayB,
		...
		if(test->new_accuracy)
		{
		    bblas_cgemm_tolerance(test, test->other_result);
		} else
		{
