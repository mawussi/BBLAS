/**
 * @file testing_zsyr2k_batch.c
 *
 * @brief Testing zsyr2k_batch double _Complex routine.
 *
...
void testing_zsyr2k_batch(bblas_ztest_t *test){
	...
	switch (test->target)
	{
	    ...
	    default:
		printf("In testing_zsyr2k_batch(): Target no defined\n");
		exit(EXIT_FAILURE);
	}
	...
	bblas_zsyr2k_batch(test->uplo, test->trans, test->N, test->K,
	...
	switch(test->target)
	{
		...
		{
		    bblas_zsyr2k_tolerance(test, test->mkl_result);
		} else
		{
		...
		if(test->new_accuracy)
		{
		    bblas_zsyr2k_tolerance(test, test->device_result);
		} else
		{
		...
		********************************/
		...
		omp_zsyr2k_batch(test->uplo, test->trans, test->N, test->K,
			    test->alpha, (const BBLAS_Complex64_t**)test->arrayA,
			    test->lda, (const BBLAS_Complex64_t**)test->arrayB, test->ldb,
		...
		if(test->new_accuracy)
		{
		    bblas_zsyr2k_tolerance(test, test->other_result);
		} else
		{
