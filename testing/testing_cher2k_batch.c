/**
 * @file testing_cher2k_batch.c
 *
 * @brief Testing cher2k_batch float _Complex routine.
 *
...
/**
 * Code generation
 * @generated from testing_zher2k_batch.c normal z -> c, Mon Jun  6 09:44:13 2016
 **/
#endif
...
void testing_cher2k_batch(bblas_ctest_t *test){
	...
	switch (test->target)
	{
	    ...
	    default:
		printf("In testing_cher2k_batch(): Target no defined\n");
		exit(EXIT_FAILURE);
	}
	...
	bblas_cher2k_batch(test->uplo, test->trans, test->N, test->K,
		    test->alpha, (const BBLAS_Complex32_t**)test->arrayA,
		    test->lda, (const BBLAS_Complex32_t**)test->arrayB, test->ldb,
		    test->beta_herk, test->arrayC, test->ldc,
	...
	switch(test->target)
	{
		...
		bblas_cher2k_batch(test->uplo, test->trans, test->N, test->K,
		    test->alpha, (const BBLAS_Complex32_t**)test->arrayA,
		    test->lda, (const BBLAS_Complex32_t**)test->arrayB, test->ldb,
		    test->beta_herk, test->mkl_result, test->ldc,
		    test->batch_count, test->batch_opts, test->info);
		...
		{
		    bblas_cher2k_tolerance(test, test->mkl_result);
		} else
		{
		...
		bblas_cher2k_batch(test->uplo, test->trans, test->N, test->K,
			    test->alpha, (const BBLAS_Complex32_t**)test->arrayA,
			    test->lda, (const BBLAS_Complex32_t**)test->arrayB, test->ldb,
			    test->beta_herk, test->device_result, test->ldc,
			    test->batch_count, test->batch_opts, test->info);
		...
		if(test->new_accuracy)
		{
		    bblas_cher2k_tolerance(test, test->device_result);
		} else
		{
		...
		********************************/
		...
		omp_cher2k_batch(test->uplo, test->trans, test->N, test->K,
			    test->alpha, (const BBLAS_Complex32_t**)test->arrayA,
			    test->lda, (const BBLAS_Complex32_t**)test->arrayB, test->ldb,
			    test->beta_herk, test->other_result, test->ldc,
			    test->batch_count, test->batch_opts, test->info);
		...
		if(test->new_accuracy)
		{
		    bblas_cher2k_tolerance(test, test->other_result);
		} else
		{
