/**
 * @file bblas_stesting.c
 *
 * @brief BBLAS testing  for float routines.
 *
...
/**
 * Code generation
 * @generated from bblas_ztesting.c normal z -> s, Mon Jun  6 09:44:14 2016
 **/
#endif
...
void bblas_ssettest(bblas_stest_t *test)
{
    ...
    {
	bblas_ssetbeta_herk(test);
    }

    /*
     * Allocate memory and set values for arrayA
     */
    bblas_ssetarrayA(test);
	...
	(routine == BBLAS_HER2K))
    {
	bblas_ssetarrayC(test);
    }

    /* Memory allocation for result and error variables */
    bblas_smalloc_result_error(test);
}
...
void bblas_ssetalpha(bblas_stest_t *test)
{
    ...
    test->alpha = (float*) malloc(batch_count*sizeof(float));
    ...
    {
	test->alpha[batch_iter] = ((float)rand()/(float)RAND_MAX);
    }
}
...
void bblas_ssetbeta(bblas_stest_t *test)
{
    ...
    test->beta = (float*) malloc(batch_count*sizeof(float));
    ...
    for( batch_iter =0; batch_iter < batch_count; batch_iter++)
    {
	test->beta[batch_iter] = ((float)rand()/(float)RAND_MAX);
    }
}
...
void bblas_ssetarrayA(bblas_stest_t *test)
{
    int batch_iter, nb_row, nb_col;
     ...
     test->arrayA = (float**) malloc(batch_count*sizeof(float*));
	 ...
	 for( batch_iter =0; batch_iter < batch_count; batch_iter++)
	 {
	     ...
	     /*Matrix filling */
	     test->arrayA[batch_iter] = (float *) malloc(nb_row*nb_col* sizeof(float ));
	     bblas_malloc_check(test->arrayA[batch_iter], ptr_name);

#if defined(BBLAS_WITH_MKL)
	     LAPACKE_slarnv_work(IONE, ISEED, nb_row*nb_col, (float*) test->arrayA[batch_iter]);
#else
		 LAPACKE_slarnv_work(IONE, ISEED, nb_row*nb_col, test->arrayA[batch_iter]);
#endif
...
void bblas_ssetarrayB(bblas_stest_t *test)
{
    ...
    test->arrayB = (float**) malloc(batch_count*sizeof(float*));
	...
	for( batch_iter =0; batch_iter < batch_count ; batch_iter++)
	{
	    ...
	    test->arrayB[batch_iter] = (float *) malloc(nb_row*nb_col* sizeof(float ));
	    bblas_malloc_check(test->arrayB[batch_iter], ptr_name);

#if defined(BBLAS_WITH_MKL)
	    LAPACKE_slarnv_work(IONE, ISEED, nb_row*nb_col, (float*) test->arrayB[batch_iter]);
#else
		LAPACKE_slarnv_work(IONE, ISEED, nb_row*nb_col, test->arrayB[batch_iter]);
#endif
	    ...
	    {
#if defined(BBLAS_WITH_MKL)
		test->Binitnorm[batch_iter] = (float) LAPACKE_slange_work(LAPACK_COL_MAJOR,
								  'I', nb_row, nb_col,
								  (float*) test->arrayB[batch_iter],
								  test->ldb[batch_iter], work);
#else
		test->Binitnorm[batch_iter] = LAPACKE_slange_work(LAPACK_COL_MAJOR,
								  'I', nb_row, nb_col,
...
void bblas_ssetarrayC(bblas_stest_t *test)
{
    ...
    test->arrayC = (float**) malloc(batch_count*sizeof(float*));
	...
	for( batch_iter =0; batch_iter < batch_count ; batch_iter++)
	{
	    ...
	    test->arrayC[batch_iter] = (float *) malloc(nb_row*nb_col* sizeof(float ));
	    bblas_malloc_check(test->arrayC[batch_iter], ptr_name);
#if defined(BBLAS_WITH_MKL)
	    LAPACKE_slarnv_work(IONE, ISEED, nb_row*nb_col, (float*) test->arrayC[batch_iter]);
#else
		LAPACKE_slarnv_work(IONE, ISEED, nb_row*nb_col, test->arrayC[batch_iter]);
#endif
...
#if defined(BBLAS_WITH_MKL)
	    test->Cinitnorm[batch_iter] = (float) LAPACKE_slange_work(LAPACK_COL_MAJOR,
								  'I', nb_row, nb_col, (float*) test->arrayC[batch_iter],
								  test->ldc[batch_iter], work);
#else
		test->Cinitnorm[batch_iter] = LAPACKE_slange_work(LAPACK_COL_MAJOR,
...
void bblas_smalloc_result_error(bblas_stest_t *test)
{
    ...
    {
	case BBLAS_MKL:
	    test->mkl_result  = (float**) malloc(batch_count*sizeof(float*));
	...
	case BBLAS_CUBLAS:
	case BBLAS_MAGMA:
	    test->device_result = (float**) malloc(batch_count*sizeof(float*));
	...
	case BBLAS_OTHER:
	    test->other_result  = (float**) malloc(batch_count*sizeof(float*));
...
void bblas_scopy_Cinit(bblas_stest_t *test, float **C_copy)
{
    ...
    if (!((routine == BBLAS_GEMM) || (routine == BBLAS_SYMM) ||
	  ...
	  (routine == BBLAS_HER2K)))
    {
	printf("BBLAS FATAL ERROR: bblas_stesting.c():\n");
	printf("\t bblas_scopy_Cinit not defined for %s \n", bblas_getroutine(test->routine));
	exit(EXIT_FAILURE);
    }
	...
	for( batch_iter =0; batch_iter < batch_count ; batch_iter++)
	{
	    ...
	    N   = test->N[batch_iter];
	    C_copy[batch_iter] = (float *) malloc(ldc*N* sizeof(float ));

	    /*Copy the matrix {C}_i */
	    cblas_scopy (ldc*N, test->arrayC[batch_iter], 1, C_copy[batch_iter], 1);
	}
...
void bblas_scopy_Binit(bblas_stest_t *test, float **B_copy)
{
    ...
    {
	printf("BBLAS FATAL ERROR: bblas_stesting.c():\n");
	printf("\t bblas_scopy_Binit not defined for %s \n", bblas_getroutine(test->routine));
	exit(EXIT_FAILURE);
    }
	...
	for( batch_iter =0; batch_iter < batch_count ; batch_iter++)
	{
	    ...
	    B_copy[batch_iter] = (float *) malloc(ldb*N* sizeof(float ));

	    /*Copy the matrix {B}_i */
	    cblas_scopy (ldb*N, test->arrayB[batch_iter], 1, B_copy[batch_iter], 1);
	}
...
void bblas_scheck_Cfinal(bblas_stest_t *test, float **C_final)
{
    ...
    {
	printf("BBLAS FATAL ERROR: bblas_stesting.c():\n");
	printf("\t bblas_scheck_Cfinal not defined for %s \n", bblas_getroutine(test->routine));
	exit(EXIT_FAILURE);
    }
...
void bblas_scheck_Bfinal(bblas_stest_t *test, float **B_final)
{
    ...
    {
	printf("BBLAS FATAL ERROR: bblas_stesting.c():\n");
	printf("\t bblas_scheck_Bfinal not defined for %s \n", bblas_getroutine(test->routine));
	exit(EXIT_FAILURE);
    }
	...
	for( batch_iter =0; batch_iter < batch_count ; batch_iter++)
	{
	    ...
	    Bnorm = cblas_snrm2(ldb*N, test->arrayB[batch_iter], 1);

	    /*Compute the error */
	    cblas_saxpy (ldb*N, (alpha), B_final[batch_iter], 1, B_diff[batch_iter], 1);

	    /*Compute  the norm assoicated  with the error */
	    Error_norm = cblas_snrm2(ldb*N, B_diff[batch_iter], 1);
		...
		case BBLAS_OTHER:
		    ...
		    break;
		default:
		    printf("In bblas_scheck_Bfinal(): Target no defined\n");
		    exit(EXIT_FAILURE);
	    }
	}
...
void bblas_sfreetest( bblas_stest_t *test )
{
	...
	default:
	    printf("In bblas_sfreetest(): Target no defined\n");
	    exit(EXIT_FAILURE);
    }
    /*Free INFO */
    free( test->info );
