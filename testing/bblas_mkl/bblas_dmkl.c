 ...
 * @file bblas_dmkl.c
 *
 * @brief BBLAS MKL routines for double precision.
 *
...
/**
 * Code generation
 * @generated from ./bblas_mkl/bblas_zmkl.c normal z -> d, Mon Jun  6 09:44:13 2016
 **/
#endif
...
/**
 * Wrapper for dgemm_batch in MKL.
 *
 * Calls cblas_dgemm_batch (linked to MKL) with the input parameters and prints out
					 ...
					 enum BBLAS_OPTS batch_opts, int *info)
{
	...
	if (batch_opts == BBLAS_VARIABLE)
	{
		...
		/* Call MKL */
		cblas_dgemm_batch( BblasColMajor,
						   (CBLAS_TRANSPOSE*) transA, (CBLAS_TRANSPOSE*) transB,
						   (const MKL_INT*) M,
						   (const MKL_INT*) N,
						   (const MKL_INT*) K,
						   (const double*) alpha,
						   (const double**) arrayA, (const MKL_INT*) lda,
						   (const double**) arrayB, (const MKL_INT*) ldb,
						   (const double*) beta,
						   (double**) arrayC, (const MKL_INT*) ldc,
	...
	}else if (batch_opts == BBLAS_FIXED)
	{
		...
		printf("ERROR: mkl_dgemm_batch called with invalid batch_opts parameter.\n");
	}
#endif
}
